...
float Ki =    0;                                // PID integral control gain
...
int last_error=0, throt_input = 70; 
int updatePid(int command, int targetValue, int currentValue)   {             // compute PWM value
float pidTerm = 0;                                                            // PID correction
int error=0;                                                             
  error = abs(targetValue) - abs(currentValue); 
  pidTerm = error*Ki + U_I;
  U_I = pidTerm;
  pidTerm += (Kp * error) + (Kd * (error - last_error));                            
  last_error = error;
  return constrain(int(pidTerm), -120, 120);
}

int correctPID(int in, int minVal, int maxVal)   
{ 
  if(in < minVal)
    return minVal;
  if(in>maxVal)
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  int pwm1 = throt_input;
  int pwm2 = throt_input;
  
  int pwm = updatePid(throt_input, angle_input, int(angle_x));
  
  //int pwm1 = pwm;
  //int pwm2 = pwm1/2;
  //if((abs(0) - abs(angle_x))>0) 
  if(angle_x<0)
  {
    pwm2+=pwm;
    pwm1-=pwm;
  ...
  else
  {
    ...
    pwm1 = correctPID(pwm1, 5, 160);
    pwm2 = correctPID(pwm2, 5, 160);  
    analogWrite(M1, pwm1);
    analogWrite(M2, pwm2);
/*if(abs(abs(angle_input) - abs(angle_x))<4)
{
  //pwm2= pwm1 = 20;
  analogWrite(M1, 20);
  analogWrite(M2, 20);
...
void processPID() {
    String sdata[4];
    split(sdata,4,inputString,'%');
    Kp = Str2int(sdata[1])/1000.0;
    Ki = Str2int(sdata[2])/1000.0;
    Kd = Str2int(sdata[3])/1000.0;
    U_I = 0;
    last_error = 0;
}
...
void processAngle() {
    String sdata[2];
    split(sdata,2,inputString,'%');
    angle_input = Str2int(sdata[1]);
}

void processThrott() {
    String sdata[2];
    split(sdata,2,inputString,'%');
    throt_input = Str2int(sdata[1]);
} 
